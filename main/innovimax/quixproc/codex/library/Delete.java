...
public class Delete extends DefaultStep implements MatchHandler {
    ...
    private QuixEvent matchedRoot = null;
    ...
    public void gorun() {             
        ...
        try {             
            ...
            runtime.getTracer().debug(step,null,-1,source,null,"  DELETE > RUN MATCH THREAD");                    
            running = true;
            t.start();                                           
        }         
        catch (Exception e) {            
        ...
        try {
            ...
            switch (event.getType()) {
                ...
                case END_SEQUENCE :
                  ...
                  break;
                case START_ELEMENT :
                    if (!delete&&match.isMatched()) {                     
                        delete = true;
                        matchedRoot = event;
                        matchedStack.push(event);
                    } 
                    else if (delete) {
                        if (event.asStartElement().getQName().equals(matchedRoot.asStartElement().getQName())) {
                            matchedStack.push(event);
                        }                    
                    }
                    break;
                case END_ELEMENT :  
                    if (match.isMatched()) {
                        if (event.asEndElement().getQName().equals(matchedRoot.asStartElement().getQName())) {
                            matchedStack.pop();
                    ...
                    break;
                case ATTRIBUTE :  
                    if (!delete&&match.isMatched()) { delete = true; }
                    break;
                case PI :  
                    if (!delete&&match.isMatched()) { delete = true; }
                    break;
